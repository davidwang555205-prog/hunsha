import type { BridalImageKeywordProfileId } from "../../src/data/bridalImageKeywordProfiles";
import {
...
import { isSceneCompatibleWithImageType } from "../../src/data/bridalDressSceneOptions";
import type { ImageType, ModelChoice, ProductCategory, PromptParams, ScenePreference } from "../../src/types";
...
// 活动主题覆盖：默认代码内置 xiaohongshuTopicOverrides，可由后台 content_engines.config.topicOverrides
// 运行时覆盖（setTopicOverrides）。拉不到/为空则用代码默认，永不阻塞生图。
let activeTopicOverrides: typeof xiaohongshuTopicOverrides = xiaohongshuTopicOverrides;

/** 运行时设置主题覆盖（后台 content_engines.config.topicOverrides）。传空/无效重置为代码默认。 */
export function setTopicOverrides(overrides?: unknown) {
  if (!overrides || typeof overrides !== "object" || Object.keys(overrides as Record<string, unknown>).length === 0) {
    activeTopicOverrides = xiaohongshuTopicOverrides;
    return;
  }
  activeTopicOverrides = { ...xiaohongshuTopicOverrides, ...(overrides as typeof xiaohongshuTopicOverrides) };
}
...
function buildCopyVariationBank(topic: FashionSeedingTopic, kit: TopicCopyKit): CopyVariationBank {
  ...
  const override = activeTopicOverrides[topic] ?? {};
...
// 素材导出工具依赖（scripts/export-seeding-assets.test.ts 用），保留以支持 assets.json 重新生成
export {
  bridalVariationBank,
  dressVariationBank,
